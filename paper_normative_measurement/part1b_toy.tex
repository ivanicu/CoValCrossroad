...
\end{theorem}
\begin{proof}
\emph{Step 1.} A decision rule based on $G$ is a map $d_G : G \mapsto \mathcal{A}$.

\emph{Step 2.} Composing with the kernel, $d_G \circ K$ is a (possibly
randomised) decision rule based on $Z$: given $z$, draw $g \sim K(\cdot\mid z)$
and play $d_G(g)$.

\emph{Step 3.} By construction the two procedures induce the same joint
distribution over $(\theta, \text{action})$, hence the same expected loss.

\emph{Step 4.} Therefore every risk attainable with $G$ is attainable with $Z$;
...
\end{theorem}
\begin{proof}[Reduction to a checkable core]
\emph{Step 1.} The \emph{unanimity games} $u_T$, defined by $u_T(S)=1$ if
...
\emph{Step 2.} On $u_T$, axioms (E), (S), (N) force the value uniquely: players
...
\emph{Step 3.} By (L), $\phi(v) = \sum_T \lambda_T \phi(u_T)$, which is now
