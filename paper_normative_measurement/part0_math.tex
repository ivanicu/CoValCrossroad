...
\step{Derivation, three sub-steps.} From \cref{thm:clt}, with probability
$0.95$: $-1.96\le(\bar X-\mu)/\se\le1.96$. Multiply by $\se>0$ (inequalities
preserved). Subtract $\bar X$ and multiply by $-1$ --- which \textbf{reverses}
both inequalities --- giving
...
\step{The model.} $X_{pr}=\mu+a_p+e_{pr}$ with $\Var(a_p)=\sigma_b^2$
(``between prompts'') and $\Var(e_{pr})=\sigma_w^2$ (``within a prompt'').

\step{The correct variance.} Cluster means are independent, so \cref{eq:varmean}
applies \emph{to them}:
\begin{equation}
