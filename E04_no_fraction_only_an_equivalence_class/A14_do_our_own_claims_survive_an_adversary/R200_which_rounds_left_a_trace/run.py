...
# parents[1] is E04, not the repo root -- the SECOND time this exact off-by-one has
...
def main() -> int:
    ...
    dirs = sorted(p.parent.name for p in (ROOT / "E04").glob("r*/run.py"))
    ...
    for k in sorted(silent, key=int):
        res = list((ROOT / "E04" / nums[k] / "results").glob("*.json")) \
            if (ROOT / "E04" / nums[k] / "results").exists() else []
        doc = ""
        try:
            src = (ROOT / "E04" / nums[k] / "run.py").read_text()
