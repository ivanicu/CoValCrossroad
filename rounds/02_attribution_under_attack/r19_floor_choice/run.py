...
def main() -> None:
    ...
    ap.add_argument("--source", type=Path,
                    default=_ROOT / "rounds/02_attribution_under_attack/r10_attribution_robustness/results/a10_attribution.json")
    ap.add_argument("--out", type=Path, default=_RES / "r19_floor_choice.json")
    a = ap.parse_args()

    if not a.source.exists():
        raise SystemExit(f"missing {a.source} -- run rounds/02_attribution_under_attack/r10_attribution_robustness/run.py first")
