...
R58 = _ROOT / "rounds/06_the_judges_mechanism/r58_equivalence_census/results/r58_equivalence_census.json"
...
def main() -> None:
    ...
    for rnd in sorted({r["round"] for r in unv}, key=lambda x: int(x[1:])):
        ...
        src = next((p for p in _ROOT.glob(f"rounds/{rnd}_*/run.py")), None)
