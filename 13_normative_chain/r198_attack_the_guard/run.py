...
def main() -> int:
    ...
    print(f"  prompt-weighted value is {b['group']:.1%}, a gap of {abs(b['gap']):.1%}. The two units do")
    print(f"  NOT agree -- they differ by more than three times the tolerance -- and both are real")
    print(f"  numbers about different things. The guard firing does not mean either is wrong. It")
    print(f"  means the sentence 'the longest response is ranked first 37.3% of the time' never")
    print(f"  said 37.3% of WHAT, and the honest version names the unit: 37.3% of assessments,")
    print(f"  35.6% of prompts.")
    print(f"  The headline conclusion survives either way, because both clear the 25% null by a")
    print(f"  wide margin. What did NOT survive was the STRATIFIED version in r191, where the")
    print(f"  anchor sits in one stratum and not the other and the choice flipped the finding.")
    ...
    from covalx.estimand import TOL
    print(f"\n  AND TOL = {TOL} IS A CHOICE, NOT A MEASUREMENT -- half a percentage point on a 0-1")
    print(f"  quantity. It is stated as chosen in the source rather than justified, because")
    print(f"  pretending a picked constant was derived is the failure this project has caught in")
    print(f"  others four times. Note it is imported here rather than retyped: the first draft of")
    print(f"  this paragraph hardcoded the OLD threshold and went stale the moment the guard was")
    print(f"  rewritten, which is the same defect as the tally r175 found in DEFECTS.py.")
