...
/* ---- 01c the people ---- */
"s1c.h":   ["The 1,012 people, positioned by how they actually voted","1,012 个人,按他们实际怎么投票来定位"],
"s1c.sub": ["Position is each rater's own <b>vote vector</b> — the six pairwise signs they gave on every prompt they saw — so two people sit near each other when they <b>ranked alike on the prompts they shared</b>. 98.4% of pairs share at least one prompt. <span class='hot'>Colour is demographics. If demography organised this panel, the colours would band.</span> Hover for a rater; click to read everything they wrote.",
            "位置来自每个人自己的<b>投票向量</b> —— 他们在看过的每条 prompt 上给出的六个两两符号 —— 所以两个人靠得近,意味着他们<b>在共同看过的题目上排得像</b>。98.4% 的两两之间至少有一条共同 prompt。<span class='hot'>颜色是人口学。如果人口学真的组织了这个面板,颜色就会成带。</span>悬停看一个人,点击读他写过的全部。"],
"s1c.c1":  ["Does any shipped attribute mark a values bloc? — R183, reused","有任何一个字段标出了价值观集团吗? —— 复用 R183"],
"s1c.c2":  ["Do stated values predict revealed choices? — R03, reused","嘴上说的能预测手上选的吗? —— 复用 R03"],
"df.age":                  ["age","年龄"],
"df.gender":               ["gender","性别"],
"df.country_of_residence": ["country","居住国"],
"df.education_level":      ["education","教育程度"],
"df.generative_ai_usage":  ["AI usage","AI 使用频率"],
"df.ai_concern_level":     ["AI concern","对 AI 的态度"],
"pe.prov":     ["{S} of {N} shown · colour = {H} · {P} · size = prompts they judged","显示 {S} / {N} · 颜色 = {H} · {P} · 大小 = 他判过的题数"],
"pe.why":      ["mean rationale","平均理由长度"],
"pe.planted":  ["PLANTED bloc (positive control)","植入的假集团(阳性对照)"],
"pe.offscale": ["— off this scale","—— 已超出本图刻度"],
"pe.chance":   ["chance","随机线"],
"pe.stated":   ["stated predicts chosen","说的预测选的"],
"pe.perm":     ["permuted null","置换零假设"],
"pe.blocprov": ["{P} permutations · min group {M} raters · {R} raters · {N} levels tested · {K} clear the threshold","{P} 次置换 · 组内最少 {M} 人 · 共 {R} 人 · 测了 {N} 个 level · {K} 个过线"],
"pe.r03a":     ["difference {D} [{A}, {B}] — indistinguishable from chance","差值 {D} [{A}, {B}] —— 与随机无法区分"],
"pe.r03b":     ["{J} judgements from {A} annotators","{J} 次判断,来自 {A} 位标注者"],
"pe.blocnote": ["<b>⚠ Two things this plot must say about itself.</b> <b>(1)</b> Only <b>{K} of {N}</b> demographic levels clear the threshold, both countries, while the <b>planted</b> bloc — people given a fake label built from their own choices — reaches <b>+{P}</b>, off the right edge. The instrument can see a bloc; the release's demographics are not one. <b>(2)</b> R183's own <b>negative control FAILED</b> its pre-registered bound of {BD}: shuffled labels reached {F} at the 95th percentile, so the resolution floor was raised to that value rather than the number originally promised. Carried here rather than quietly dropped.",
                "<b>⚠ 这张图必须说出关于自己的两件事。</b><b>(1)</b> 只有 <b>{N} 个 level 里的 {K} 个</b>过线,而且都是国家;而<b>植入的</b>假集团 —— 用一个人自己的选择造出来的假标签 —— 达到 <b>+{P}</b>,已经跑出右边界。**仪器看得见集团;这个 release 的人口学不是集团。** <b>(2)</b> R183 自己的<b>负控制没通过</b>预注册的 {BD} 界:随机打乱的标签在 95 分位达到 {F},所以分辨率地板被抬高到这个值,而不是当初承诺的那个数。这里如实带出来,没有悄悄丢掉。"],
"pe.who":      ["who this is","这是谁"],
"pe.didn":     ["prompts judged","判过的题数"],
"pe.ideal":    ["what they said ideal model behaviour is","他自己写的「理想的模型行为」"],
"pe.every":    ["everything they judged and wrote — {N} prompts","他判过和写过的全部 —— {N} 条"],
