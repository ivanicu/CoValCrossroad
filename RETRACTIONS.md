...
| 28 | **That the sign test had returned an answer at all** | It returned **+1.40, +2.26, +2.68 and +10.26** on identical data, varying only with how many random half-splits were averaged and whether the null used the same number. Four answers to one question | The diagnosis: that is a reading of the estimator, not a measurement. [r26](E02_the_plural_public_dissolved/A04_structured_plurality_or_reliability/R26_sign_no_split) removes the split entirely and works on each pair's full residual series |
...
| 39 | **"effort steps −38.6% at task 6 — a regime change"** — r02, defended by r24 | `DATASET_CARD.md:81`: *"a minimum of 5 tasks and up to 20 per session"*. **The breakpoint is the study's continuation boundary.** Positions 1–5 hold everyone who started; 6 holds only those who continued. And r24's null shuffles positions globally, which **destroys the censoring process that creates the confound** — it could not have detected this | [r31](E02_the_plural_public_dissolved/A04_structured_plurality_or_reliability/R31_within_person) runs the repair. On the **933 people present at both**, the paired drop is **−179.2 chars [−196.2, −162.3], −53.3%**, against **6.1% attrition**; between-person is −53.2%, essentially identical. **Composition is excluded.** But the mechanism is not: with sessions of 5 or 15 prompts and **no session identifier or timestamp anywhere in the release** (verified — the only assessment fields are annotator_id, conversation_id, importance, ranking_blocks, representativeness, subjectivity), position 6 is the **first task of a later session** for anyone whose first batch held five. Within-session fatigue and between-session habituation are **not separable from this data** |
...
`E02_the_plural_public_dissolved/A05_what_coval_core_mechanically_is/R34_global_rater_crossfit/run.py:132`, `if len(sc) >= thr` — keeping only criteria rated
...
0.1592 and 0.2880 appear in no artifact, and [r66](E03_the_instrument_was_the_object/A08_what_floor_each_counterfactual_requires/R66_r56_reconstruction) could not
...
**`a06_dryrun.json`** sits in `E01_the_rubric_was_the_object/A02_which_aggregation_rule/R06_rule_tournament/results/`. Its name contains no form of
...
`Judge(MODEL_DIR, batch=32)` (`E01_the_rubric_was_the_object/A03_is_the_attribution_real_and_against_what_floor/R12_response_set/run.py:208`). Human rankings replace the
...
**`E02_the_plural_public_dissolved/A06_does_the_protocol_have_the_power_it_needs/R44_compiler_lineage/run.py:112` states the limitation in its own comment:**
...
the count found something worse: **`E03_the_instrument_was_the_object/A07_can_a_local_judge_be_an_instrument/R56_semantic_selectivity/` contains one file,
...
including [r66](E03_the_instrument_was_the_object/A08_what_floor_each_counterfactual_requires/R66_r56_reconstruction), which imports from another round's module and needs
...
**`E02_the_plural_public_dissolved/A06_does_the_protocol_have_the_power_it_needs/R39_feature_cache/results/r39_feature_cache.npz` is 57 MB and tracked in git.** It holds
...
**A repository note found on the way.** `E03_the_instrument_was_the_object/A07_can_a_local_judge_be_an_instrument/R54_overlap_transfer/run.py` defaults `--gen` to
...
**What was sitting there.** `E03_the_instrument_was_the_object/A07_can_a_local_judge_be_an_instrument/R47_gold_is_length/results/r47_gold_is_length.json` →
...
there never was.** [r80](E03_the_instrument_was_the_object/A09_can_direction_be_read_from_text_alone/R80_panel_freeze) separates two things the phrase conflates:
...
**The queue's GPU item, delivered.** [r80](E03_the_instrument_was_the_object/A09_can_direction_be_read_from_text_alone/R80_panel_freeze) records every element it names —
...
**⚠ CORRECTION from [r81](E03_the_instrument_was_the_object/A09_can_direction_be_read_from_text_alone/R81_stated_signal_by_length): the split below is in the NULL, not
...
> it. [r35](E02_the_plural_public_dissolved/A05_what_coval_core_mechanically_is/R35_polarity_abstention) carries a `scale_note` recording exactly this: *"the value
...
> make**. What r82 actually added is measured and narrower: the full histogram, the **29.55%**
> concentration on 5 or 10, and the provenance split (**19.21%** vs **6.98%**, +0.1223). The midpoint
> fact was already in the package.
...
**The released weight distribution had never been read out in full.** r61 mentioned the neutral point
while computing power, and ⚠ **r35 had already stated the midpoint fact outright** — see the correction
above. What was missing was the shape: the histogram, the concentration, and the provenance split.
...
**It exists.** [r32](E02_the_plural_public_dissolved/A04_structured_plurality_or_reliability/R32_channel_decomposition) has the cell:
...
## Entry 150 — I ran the prior-art gate and searched for the wrong words, so I claimed a fact was new that was already in the package
...
**It had.** [r35](E02_the_plural_public_dissolved/A05_what_coval_core_mechanically_is/R35_polarity_abstention) stores a `scale_note` saying it verbatim: *"the value
...
**And I did run the gate.** Before building r82 I grepped the rounds for `magnitude`, `abs(score)`,
`score distribution` and `histogram`. Every one of those describes **how I intended to measure it**.
None of them is **the claim I was about to make**. r35 records the fact in prose, in a field named
`scale_note`, using none of my search terms — and it would have taken one grep for `102,147` or
`neutral point` to find it.

**What r82 actually contributed, delimited by measurement rather than by assertion.** Checking r35's
note for each of r82's numbers: it contains `102,147` and `once`; it does **not** contain 29.55%,
16.96%, `provenance`, `seed`, `write-in`, or `5 or 10`. So the new material is the **shape** — the full
histogram, the **29.55%** concentration on two of twenty-one values, and the provenance split
(**19.21%** vs **6.98%**, +0.1223 against a permutation null). That is a real contribution and it is
smaller than what I claimed.

**Corrected in four places**, because the overstatement had already propagated: entry 146's title and
body, r82's verdict (regenerated by re-running, not typed), and the README row.

**The rule this refines.** The prior-art gate says *ask the system, don't read a document*. I asked the
system — with the vocabulary of my **method** instead of the vocabulary of my **finding**. A search
built from how you plan to measure something will systematically miss anyone who measured it
differently and wrote it down plainly. **Search for the claim, then for the method** — and the claim is
the cheaper query, because a fact has fewer synonyms than a technique.

**Third prior-art event this session, and they are not the same.** Entry 147: the gate **worked** and
...
`E01_the_rubric_was_the_object/A03_is_the_attribution_real_and_against_what_floor/R12_response_set/results/a12_fresh_generations.json` — **r12's 250**. So the chain is:
...
|---|---|---|
| real prompt-specific rubric **0.686** | **0.6879** long-form, **0.6854** short-form | [r85](E03_the_instrument_was_the_object/A10_what_the_resampling_unit_is/R85_agreement_by_form) |
| the split of the 18.6 points is **one cell of a grid spanning 3.2%–65.8%** | a new cell at **65.3%** | [r86](E03_the_instrument_was_the_object/A10_what_the_resampling_unit_is/R86_attribution_by_form) |
...
|---|---:|---|
| **whole best-to-worst aggregation-rule span** ([r06](E01_the_rubric_was_the_object/A02_which_aggregation_rule/R06_rule_tournament)) | **0.0188** | **42% of the margin** |
| containment asymmetry, residualised ([r50](E03_the_instrument_was_the_object/A07_can_a_local_judge_be_an_instrument/R50_response_anchoring)) | +0.0203 | 46% |
| write-in vs seeded transfer gap ([r49](E03_the_instrument_was_the_object/A07_can_a_local_judge_be_an_instrument/R49_provenance_crossfit)) | +0.0172 | 39% |
| compatibility selection vs size-matched random ([r44](E02_the_plural_public_dissolved/A06_does_the_protocol_have_the_power_it_needs/R44_compiler_lineage)) | +0.0149 | 33% |
...
**Wrong, and wrong in the expensive direction.** `E01_the_rubric_was_the_object/A03_is_the_attribution_real_and_against_what_floor/R22_cross_family/results/r22_cross_family_per_prompt.npz`
...
|---|---|---|
| **no vector was stored** though the estimand is a mean of paired units | r06's rule contrasts, resolved in [r97](E03_the_instrument_was_the_object/A10_what_the_resampling_unit_is/R97_rule_tournament_tost) | **yes** |
...
- **[r98](E03_the_instrument_was_the_object/A10_what_the_resampling_unit_is/R98_unverified_triage)** uses `(round, path, file)` purely as an **identity tuple** for
...
I first grepped `E02_the_plural_public_dissolved/A06_does_the_protocol_have_the_power_it_needs/R45_protocol_freeze/run.py` and got *"No such file or directory"* — and was one
