...
| 25 | **Entry 6's sharp form** — "seed criteria carry attribution (CI excludes 0) and write-ins do not (CI includes 0)", the asymmetry that made the response-set mechanism look dead rather than merely weakened | Entry 15's own repair. Under the corrected per-prompt pairing, **write-ins also clear zero: +0.0262 [+0.0022, +0.0506]**. Criteria authored *after* reading the four candidates carry real prompt-specific attribution too | The ordering, more strongly than before: seed **+0.0457** against write-in **+0.0262**, a gap of **+0.0194** where the broken computation showed +0.0102. Criteria written **without sight of the responses** still carry more. So the mechanism-kill stands and its sharp form does not — the correct claim is *"response-set knowledge is not what carries the attribution"*, not *"write-ins carry nothing"*. **The gap itself has no interval yet**: the two arms sit on different prompt subsets (300 vs 293), so a CI on the difference needs the intersection, and until that runs the +0.0194 is a point estimate quoted without its uncertainty — the exact habit this ledger exists to catch |
