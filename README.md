...
| [r03](rounds/r03_stated_vs_revealed) | Do stated ideals predict own choices? | **equivalent to chance at the preregistered δ=0.01**, which is stronger than the *"no evidence"* this row used to report. Over **11,327** judgements from **1,007** annotators: hit rate **0.5033** vs a label-permuted null of **0.5016**, difference **+0.0017** [−0.0061, +0.0097] — the interval lies inside the margin with **0.0003** to spare, so this is a *tight* null, not an unpowered one. (95% interval used where TOST asks for 90%: conservative, since the 90% is narrower.) ⚠ **The aggregate averages two subpopulations that do not behave alike**: the top pick is the longer response in **55.5%** of judgements, and the hit rate splits **0.5138** when it is longer against **0.4903** when it is shorter, a gap of **+0.0235** that this round does not test for significance |
