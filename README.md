...
| [r112](rounds/r112_value_idiosyncrasy) | Compilation improves nearly everyone but returns **less** to some. Is that person **describable**? | **yes — and the covariate had been on disk, unopened, for 111 rounds.** `coval_full[*].scores` carries every rater's own signed −10..+10 importance weights on individual criteria. Regressing the paired per-cell gain `e_core − e_full` on that rater's divergence from the **leave-one-out** consensus, with **prompt fixed effects** (which absorb every prompt-level quantity, so the exposure confound is *designed out* rather than adjusted for) and two-way clustering on prompt and rater, over **14,637 of 15,202** cells: **β = +0.04209** [+0.03028, +0.05391], t 6.98. Positive = **smaller gain**. **+1 sd of value divergence withholds 29.3% of the mean gain.** A within-prompt permutation of *whole score profiles* nulls at [−0.00833, +0.00737], **p_perm 0.0000**; a planted 0.30 coefficient recovers at +0.2991 | ⚠ **x1 rests on ≤6 points**: CoVal gives each rater a bounded set of *shared* criteria plus their own contributed criteria, **which nobody else scores** — so a rater's self-invented criteria, the purest statement of their values, can never enter a divergence-from-consensus measure ⚠ Split-half reliability **0.1688**, so **+0.04209 is a floor**; the deattenuated +0.14574 fixes the bias's *sign* and is marked **not quotable** ⚠ **The polarity story does not carry it**: the sign-disagreement share gives β +0.01279, p_perm 0.2500 — the mechanism is *continuous* divergence in what a person thinks matters ⚠ β is a **within-prompt** contrast across raters |
