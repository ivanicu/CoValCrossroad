...
([r88](rounds/r88_donor_draw_variance)): +0.1215 is **one donor draw**, sitting **−0.62 sd** below the
...
([r109](rounds/r109_donor_arm_is_text_blind), 2026-07-29). **Replacing every donor criterion's text
...
prompt's own criteria and varies the **weight vector** and criterion count, and [r87](rounds/r87_criterion_count_channel)
...
**No number above is wrong; the population each describes is narrower than "the release."** These ten establish their results on prompts collected with one instrument. ⚠ **"Untestable" was too strong, and [r85](rounds/r85_agreement_by_form) bounds it.** No *prompt* appears under both forms, so a prompt-for-prompt contrast is impossible — but rounds using the whole join hold both populations. The rubric's agreement with **real human** rankings is **0.6879** [0.6721, 0.7034] on the 293 long-form prompts and **0.6854** [0.6738, 0.6965] on the 675 short-form ones: gap **+0.0025** [−0.0173, +0.0231], sitting inside a form-label shuffle null of [−0.0211, +0.0226]. **Not equivalence at δ=0.01** — the answerable margin here is **δ≈0.020**, limited by 293 long-form prompts. So any form-associated difference *in this quantity* is under 0.020, and the point estimate is 0.4% of the agreement level. **And [r86](rounds/r86_attribution_by_form) does the same for the quantity the package is actually about** — own-minus-donor attribution, i.e. source specificity: **+0.1258** [+0.1038, +0.1483] long-form against **+0.1202** [+0.1056, +0.1351] short-form, gap **+0.0056** [−0.0209, +0.0310], inside a shuffle null of [−0.0260, +0.0270]. **Positive and clear of zero in both forms**, with any form effect under **0.026**. So the scope block is provenance rather than a warning: those ten rounds measure their central contrast where it behaves like the rest, to that resolution. The one exception is [r46](rounds/r46_spread_replication), whose held-out set is **83.6% short-form** and which reproduces r12's inversion there (+0.0847 → −0.0716); that is the only cross-form evidence available, and it is uncontrolled. **Rounds using the whole join are unaffected** — its composition matches the file at 30.3% against 29.8%.
...
([r92](rounds/r92_writein_analysability), 2026-07-29). [r34](rounds/r34_global_rater_crossfit) applies
the majority-rated filter, so the cross-fitting above runs on the **pre-seeded** criteria — **36.5%**
of the total, and by [r48](rounds/r48_provenance_identified) exactly the six items **shown identically
...
| [r12](rounds/r12_response_set) | Does the advantage transfer? | it **inverts** off-distribution: **+0.102 [+0.071, +0.133] → −0.064 [−0.092, −0.037]**, discrimination control passed. Replicated on held-out prompts at +0.0847 → −0.0716 ([r46](rounds/r46_spread_replication)) ⚠ **One donor draw** (sd **0.0055** at n=968, [r88](rounds/r88_donor_draw_variance)) — the inversion's *magnitude* carries that, though a sign change of this size does not turn on it. |
...
| [r16](rounds/r16_minority_regret) | Conflict-aware, on its own turf | profile splits show regret 2.07 vs 1.10 random, yet conflict-aware leaves the worst-off group **lowest of all rules**. ⚠ These are **not** a demographic constituency: gender (1.145) and country (1.198) splits both fail r16's own bar ⚠ **Pre-seeded criteria only — 36.5% of the total.** This round keeps criteria rated by a *majority* of a prompt's raters; [r48](rounds/r48_provenance_identified) showed that class is the **pre-seeded set shown identically to every participant**, and [r92](rounds/r92_writein_analysability) that the excluded **63.5%** carry a median of **1** rater, so the restriction is **structural** and cannot be lifted by re-analysis. |
| [r17](rounds/r17_conditional_core) | Does conditional encoding rescue it? | **partly** — routing learned from a rater's *other* prompts helps only the rules carrying contested items (+0.195), and does not close the gap ⚠ **Pre-seeded criteria only — 36.5% of the total.** This round keeps criteria rated by a *majority* of a prompt's raters; [r48](rounds/r48_provenance_identified) showed that class is the **pre-seeded set shown identically to every participant**, and [r92](rounds/r92_writein_analysability) that the excluded **63.5%** carry a median of **1** rater, so the restriction is **structural** and cannot be lifted by re-analysis. |
| [r18](rounds/r18_routing_difficulty) | Was r17's 84.6% routing accuracy free? | **inflated by +0.147, but real**: 0.666 [0.643, 0.688] where the blocs actually disagree ⚠ **Pre-seeded criteria only — 36.5% of the total.** This round keeps criteria rated by a *majority* of a prompt's raters; [r48](rounds/r48_provenance_identified) showed that class is the **pre-seeded set shown identically to every participant**, and [r92](rounds/r92_writein_analysability) that the excluded **63.5%** carry a median of **1** rater, so the restriction is **structural** and cannot be lifted by re-analysis. |
...
| [r20](rounds/r20_paraphrase_transfer) | Is the advantage content or wording? | **content** — reword every criterion and **97.4%** of the advantage survives; original−paraphrased +0.002 [−0.007, +0.011] ⚠ The random-donor arm is **one donor draw** ([r88](rounds/r88_donor_draw_variance), sd 0.0055 at n=968); the retained-advantage ratio inherits it in both terms. |
| [r21](rounds/r21_donor_distance) | Is the "near-topic" donor actually near? | **yes, near the ceiling** — it sits at the 97.86th percentile of all pairs and covers **91.6%** of the distance from a random prompt to the same question reworded ⚠ The random point on this curve is **one donor draw** ([r88](rounds/r88_donor_draw_variance)); the near and far points are deterministic and carry no draw noise. |

| [r22](rounds/r22_cross_family) | Does the attribution survive a change of judge family? | **yes, and the magnitude does not** — positive on qwen and phi with intervals clear of zero, but the prompt-specific share runs 25.3% (phi) to 53.8% (qwen2.5-3b) at a fixed floor, a **2.13× judge span** (derived as 53.8÷25.3 from [r30](rounds/r30_scope_grid)'s shares, not stored by r22) on top of r19's 2.47× floor span. The first run falsely claimed this on two Qwen judges because "family" was `name.split("-")[0]`; phi was scoreable only after a tokenizer fix ⚠ Every cell here is **one donor draw** ([r88](rounds/r88_donor_draw_variance), sd 0.0055 at n=968), so between-judge differences smaller than ~0.011 are not separated from the draw. |
...
| [r46](rounds/r46_spread_replication) | Does the spread-loss effect hold out of sample? | **no — prediction committed to git first, then falsified.** +0.0496 [−0.068, +0.169] on 250 untouched prompts against a predicted [+0.12, +0.34]. Controls passed, and **r12's inversion itself replicated**: +0.0847 original, −0.0716 fresh ⚠ **One donor draw** ([r88](rounds/r88_donor_draw_variance)) on a held-out set smaller than the join, so its draw sd exceeds 0.0055 by the 1/√n scaling [r89](rounds/r89_floor_draw_at_panel_size) verified. |
