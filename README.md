...
| [r71](rounds/r71_r67_split_variance) | Was r67's 0.657 also a single draw? | **yes, but it survives.** Re-running r67's estimator verbatim 200 times instead of once: spread loss **0.6849** (sd 0.0294), criterion-space geometry **0.6653** (sd 0.0304), mean **0.6751** against the published 0.657 — a shift of +0.018. Multiplier 1.217× not 1.234×; r41's floor 0.227→**0.224**, r46's 0.264→**0.260**. ⚠ **The pattern is the finding**: across-split volatility scales with *unreliability*. Relative spread was 5.8% for r67's predictors (raw r≈0.51), 14% for r69's (0.28), 24% for r70's drop (0.18) — so a single split is least trustworthy exactly where the correction matters most. Shuffled null re-run every draw and reported as a **maximum** (0.2016), never a mean |
