...
def _git_anchor() -> dict:
    ...
    """
    import subprocess
    here = next(p for p in pathlib.Path(__file__).resolve().parents if (p / "covalx").is_dir())
    def git(*a):
...
def main() -> None:
    ...
    ap.add_argument("--r38", type=Path,
                    default=_ROOT / "E02_the_plural_public_dissolved/A06_does_the_protocol_have_the_power_it_needs/R38_human_sampling_power/results/"
                                    "r38_human_sampling_power.json")
    ap.add_argument("--gen", type=Path,
                    default=_ROOT / "E01_the_rubric_was_the_object/A03_is_the_attribution_real_and_against_what_floor/R12_response_set/results/"
