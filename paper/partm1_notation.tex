...
\begin{proof}
\begin{steps}
\item The law of cosines for the triangle with sides $\|x\|$, $\|y\|$
and $\|x-y\|$ states
$\|x-y\|^2 = \|x\|^2+\|y\|^2-2\|x\|\|y\|\cos\theta$.
\item Expand the left side using the definition:
\[
\|x-y\|^2=\sum_i (x_i-y_i)^2=\sum_i x_i^2-2\sum_i x_iy_i+\sum_i y_i^2
=\|x\|^2-2(x\cdot y)+\|y\|^2 .
\]
\item Set the two expressions equal. The terms $\|x\|^2$ and $\|y\|^2$
cancel from both sides, leaving $-2(x\cdot y) = -2\|x\|\|y\|\cos\theta$.
\item Divide by $-2\|x\|\|y\|$.
\end{steps}
\end{proof}
...
$Cx = x - \bar x \mathbf{1}$.
\item Centring an already-centred vector changes
...
$1$.
\item The input space has dimension $k$; a one-dimensional
