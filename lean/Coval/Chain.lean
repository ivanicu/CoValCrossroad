...
/-- **my_claim** · status `settled` · D8
    ...
    length-sorting on 158 up to 6.2e-02, because Qwen3.5's gated-delta recurrence does not fully
    mask left-pad tokens. The stored tensors were produced at batch 48 in file order, an
    unregistered implementation choice under every published number. Re-scoring the ENTIRE grid
    under length-sorted batching changes every single one of 75,244 judgements -- zero
...
opaque the_judges_batching_dependence_is_real_per_judgement_and_immaterial_per_claim : Prop

/-- **my_claim** · status `settled` · D9
    ...
    rather than between raters of the same criterion is ICC(1) = 0.2698 [0.2586, 0.2809]. There
    IS a shared object -- a rater-style null that reassigns each person's own numbers across the
    criteria they rated, preserving their scale usage and extremeness exactly, returns 0.0008
    (sd 0.0012) -- but it accounts for barely a quarter of what people said mattered.
    Seventy-three percent of the variance is about WHO IS RATING. So an aggregate is a summary
    of a minority of the variance and may not be stated as the panel's view. This sizes A3
    rather than killing it, which is the more useful thing to have done. -/
opaque the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered : Prop

/-- **my_claim** · status `settled` · D8
    The polarity route rebuilt with no benchmark and no judge, after both were challenged and
    both objections landed. Verbatim retention gives a decisive but SEMANTICALLY AMBIGUOUS fact:
    0 of 3,905 negatively-rated criteria appear verbatim in core against 372 of 11,343 positive
    ones, Fisher exact one-sided p 3.26e-49. It does not establish dropping, because full's
    negative criteria state the bad behaviour affirmatively and such a line must be negated
    before it can sit in a rubric at all -- a fact about phrasing. (The reading that core cannot
    write prohibitions is separately FALSE: 21.7% of core's criteria carry prohibition markers,
    above full's own 19.1% and 19.7%.) The channel that does work is lexical presence: strip
    negation from both sides, and each full criterion's maximum Jaccard overlap against its OWN
    prompt's core criteria is 0.1246 for negatives against 0.1911 for a magnitude-matched
    positive subset, z = -32.8 over 20 draws. Their content is not there in rephrased form
    either. -/
opaque the_negative_quarters_content_is_underrepresented_in_core_without_any_model : Prop

/-- **defect** · status `refuted` · D9
    The first pass at the verbatim polarity test reported OR 0.0000 with a 95% CI of [0.0000,
    0.0000] across four specifications. Zero of 3,905 is PERFECT SEPARATION, where the logistic
    is undefined and the fit diverges; a zero-width interval at zero is the signature of that
    divergence, not a precise estimate. Replaced by Fisher's exact test (p 3.26e-49) and a
    one-sided upper bound on the rate (7.67e-04). -/
opaque defect_i_reported_a_logistic_OR_under_perfect_separation : Prop
...
/-- Experiment `r137-batch-gauge` (D8).
    ...
    while optimising for speed: the optimised judge disagreed with the reference by 4.2e-02 and
    the only behavioural difference was length-sorted batching, so the REFERENCE was re-run
    under five batchings to locate it. -/
axiom ev_the_judges_batching_dependence_is_real_per_judgement_and_immaterial_per_claim_by_r137_batch_gauge : Evidence the_judges_batching_dependence_is_real_per_judgement_and_immaterial_per_claim

/-- Experiment `r138-is-there-a-standard` (D9).
    ...
    seeds; the null is 400 permutations across the same 5 seeds. On the scale itself, which A6
    rests on: only 16.5% of ratings are exactly +/-10, 20 of the 21 available values are used,
    mean +3.29 sd 5.44 -- the scale is genuinely used and is not a two-valued vote in a 21-point
    costume, which is a point FOR A6. -/
axiom ev_the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered_by_r138_is_there_a_standard : Evidence the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered

/-- Experiment `r140-polarity-without-the-judge` (D8).
    No model is run and no benchmark is chosen. The comparison is relative, so a uniformly weak
    lexical proxy cancels. The one confound that could manufacture it runs BACKWARDS: negative
    criteria are shorter (88 chars against 101) and Jaccard divides by the union, so brevity
    should INFLATE their overlap. It is lower anyway. Limit stated rather than argued away:
    lexical overlap is a weak proxy for content and establishing more would require semantic
    matching, i.e. a second model between the claim and the data. -/
axiom ev_the_negative_quarters_content_is_underrepresented_in_core_without_any_model_by_r140_polarity_without_the_judge : Evidence the_negative_quarters_content_is_underrepresented_in_core_without_any_model

/-- Experiment `session-self-audit-2026-07-30` (D9).
    A confidence interval of exactly zero width is never an estimate. Caught by reading the
    interval rather than the point. -/
axiom ev_defect_i_reported_a_logistic_OR_under_perfect_separation_by_session_self_audit_2026_07_30 : Evidence defect_i_reported_a_logistic_OR_under_perfect_separation
...
/-- 1 independent measurement(s) establish `the_judges_batching_dependence_is_real_per_judgement_and_immaterial_per_claim`. -/
axiom the_judges_batching_dependence_is_real_per_judgement_and_immaterial_per_claim_established : Evidence the_judges_batching_dependence_is_real_per_judgement_and_immaterial_per_claim → the_judges_batching_dependence_is_real_per_judgement_and_immaterial_per_claim
theorem the_judges_batching_dependence_is_real_per_judgement_and_immaterial_per_claim_holds : the_judges_batching_dependence_is_real_per_judgement_and_immaterial_per_claim := the_judges_batching_dependence_is_real_per_judgement_and_immaterial_per_claim_established ev_the_judges_batching_dependence_is_real_per_judgement_and_immaterial_per_claim_by_r137_batch_gauge

/-- 1 independent measurement(s) establish `the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered`. -/
axiom the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered_established : Evidence the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered → the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered
theorem the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered_holds : the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered := the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered_established ev_the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered_by_r138_is_there_a_standard

/-- 1 independent measurement(s) establish `the_negative_quarters_content_is_underrepresented_in_core_without_any_model`. -/
axiom the_negative_quarters_content_is_underrepresented_in_core_without_any_model_established : Evidence the_negative_quarters_content_is_underrepresented_in_core_without_any_model → the_negative_quarters_content_is_underrepresented_in_core_without_any_model
theorem the_negative_quarters_content_is_underrepresented_in_core_without_any_model_holds : the_negative_quarters_content_is_underrepresented_in_core_without_any_model := the_negative_quarters_content_is_underrepresented_in_core_without_any_model_established ev_the_negative_quarters_content_is_underrepresented_in_core_without_any_model_by_r140_polarity_without_the_judge

/-- 1 independent measurement(s) establish `defect_i_reported_a_logistic_OR_under_perfect_separation`. -/
axiom defect_i_reported_a_logistic_OR_under_perfect_separation_established : Evidence defect_i_reported_a_logistic_OR_under_perfect_separation → defect_i_reported_a_logistic_OR_under_perfect_separation
theorem defect_i_reported_a_logistic_OR_under_perfect_separation_holds : defect_i_reported_a_logistic_OR_under_perfect_separation := defect_i_reported_a_logistic_OR_under_perfect_separation_established ev_defect_i_reported_a_logistic_OR_under_perfect_separation_by_session_self_audit_2026_07_30
...
/-- SUPPORTS (d_forward 8). instrument-dependent does not mean unbounded: this dimension is bounded at <0.001 -/
axiom the_judges_batching_dependence_is_real_per_judgement_and_immaterial_per_claim_supports_instrument_free_vs_instrument_dependent_is_the_partition_that_matters : the_judges_batching_dependence_is_real_per_judgement_and_immaterial_per_claim → instrument_free_vs_instrument_dependent_is_the_partition_that_matters
theorem instrument_free_vs_instrument_dependent_is_the_partition_that_matters_is_supported_via_the_judges_batching_dependence_is_real_per_judgement_and_immaterial_per_claim : instrument_free_vs_instrument_dependent_is_the_partition_that_matters := the_judges_batching_dependence_is_real_per_judgement_and_immaterial_per_claim_supports_instrument_free_vs_instrument_dependent_is_the_partition_that_matters the_judges_batching_dependence_is_real_per_judgement_and_immaterial_per_claim_holds

/-- SUPPORTS (d_forward 9). the standard exists and is 27% of the variance; the assumption is not false, it is
    quantified, and the quantity is small -/
axiom the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered_supports_A3_aggregation_yields_a_collective_standard : the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered → A3_aggregation_yields_a_collective_standard
theorem A3_aggregation_yields_a_collective_standard_is_supported_via_the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered : A3_aggregation_yields_a_collective_standard := the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered_supports_A3_aggregation_yields_a_collective_standard the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered_holds

/-- SUPPORTS (d_forward 8). the -10..+10 range is genuinely exercised rather than collapsed to its endpoints -/
axiom the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered_supports_A6_ratings_are_meaningful_importance_weights : the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered → A6_ratings_are_meaningful_importance_weights
theorem A6_ratings_are_meaningful_importance_weights_is_supported_via_the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered : A6_ratings_are_meaningful_importance_weights := the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered_supports_A6_ratings_are_meaningful_importance_weights the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered_holds

/-- SUPPORTS (d_forward 9). second member of the instrument-free half, and it is about the release itself -/
axiom instrument_free_vs_instrument_dependent_is_the_partition_that_matters_supports_the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered : instrument_free_vs_instrument_dependent_is_the_partition_that_matters → the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered
theorem the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered_is_supported_via_instrument_free_vs_instrument_dependent_is_the_partition_that_matters : the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered := instrument_free_vs_instrument_dependent_is_the_partition_that_matters_supports_the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered instrument_free_vs_instrument_dependent_is_the_partition_that_matters_holds

/-- SUPPORTS (d_forward 7). a shared component that is only 27% of the variance is exactly what an own-versus-stranger
    advantage of one seventh of the between-rater floor looks like from the other side -/
axiom the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered_supports_the_ratings_capture_a_shared_standard_not_personal_values : the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered → the_ratings_capture_a_shared_standard_not_personal_values
theorem the_ratings_capture_a_shared_standard_not_personal_values_is_supported_via_the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered : the_ratings_capture_a_shared_standard_not_personal_values := the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered_supports_the_ratings_capture_a_shared_standard_not_personal_values the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered_holds

/-- REFUTES (d_forward 8). a fifth route, and the first one that does not route through the instrument -/
axiom the_negative_quarters_content_is_underrepresented_in_core_without_any_model_refutes_A1_core_is_a_faithful_compilation : the_negative_quarters_content_is_underrepresented_in_core_without_any_model → ¬A1_core_is_a_faithful_compilation
theorem A1_core_is_a_faithful_compilation_is_refuted_via_the_negative_quarters_content_is_underrepresented_in_core_without_any_model : ¬A1_core_is_a_faithful_compilation := the_negative_quarters_content_is_underrepresented_in_core_without_any_model_refutes_A1_core_is_a_faithful_compilation the_negative_quarters_content_is_underrepresented_in_core_without_any_model_holds

/-- SUPPORTS (d_forward 8). same direction reached without the judge whose polarity symmetry has since failed -/
axiom the_negative_quarters_content_is_underrepresented_in_core_without_any_model_supports_core_retains_the_negative_quarter_at_one_tenth_weight : the_negative_quarters_content_is_underrepresented_in_core_without_any_model → core_retains_the_negative_quarter_at_one_tenth_weight
theorem core_retains_the_negative_quarter_at_one_tenth_weight_is_supported_via_the_negative_quarters_content_is_underrepresented_in_core_without_any_model : core_retains_the_negative_quarter_at_one_tenth_weight := the_negative_quarters_content_is_underrepresented_in_core_without_any_model_supports_core_retains_the_negative_quarter_at_one_tenth_weight the_negative_quarters_content_is_underrepresented_in_core_without_any_model_holds

/-- SUPPORTS (d_forward 9). the number was right and the statistic was wrong -/
axiom defect_i_reported_a_logistic_OR_under_perfect_separation_supports_the_negative_quarters_content_is_underrepresented_in_core_without_any_model : defect_i_reported_a_logistic_OR_under_perfect_separation → the_negative_quarters_content_is_underrepresented_in_core_without_any_model
theorem the_negative_quarters_content_is_underrepresented_in_core_without_any_model_is_supported_via_defect_i_reported_a_logistic_OR_under_perfect_separation : the_negative_quarters_content_is_underrepresented_in_core_without_any_model := defect_i_reported_a_logistic_OR_under_perfect_separation_supports_the_negative_quarters_content_is_underrepresented_in_core_without_any_model defect_i_reported_a_logistic_OR_under_perfect_separation_holds
...
#print axioms instrument_free_vs_instrument_dependent_is_the_partition_that_matters_is_supported_via_the_judges_batching_dependence_is_real_per_judgement_and_immaterial_per_claim
#print axioms A3_aggregation_yields_a_collective_standard_is_supported_via_the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered
#print axioms A6_ratings_are_meaningful_importance_weights_is_supported_via_the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered
#print axioms the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered_is_supported_via_instrument_free_vs_instrument_dependent_is_the_partition_that_matters
#print axioms the_ratings_capture_a_shared_standard_not_personal_values_is_supported_via_the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered
#print axioms A1_core_is_a_faithful_compilation_is_refuted_via_the_negative_quarters_content_is_underrepresented_in_core_without_any_model
#print axioms core_retains_the_negative_quarter_at_one_tenth_weight_is_supported_via_the_negative_quarters_content_is_underrepresented_in_core_without_any_model
#print axioms the_negative_quarters_content_is_underrepresented_in_core_without_any_model_is_supported_via_defect_i_reported_a_logistic_OR_under_perfect_separation
...
#print axioms the_judges_batching_dependence_is_real_per_judgement_and_immaterial_per_claim_holds
#print axioms the_collective_standard_exists_and_is_27_percent_of_what_people_said_mattered_holds
#print axioms the_negative_quarters_content_is_underrepresented_in_core_without_any_model_holds
#print axioms defect_i_reported_a_logistic_OR_under_perfect_separation_holds
