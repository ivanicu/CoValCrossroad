...
anything. Frozen frame: `E02_the_plural_public_dissolved/A06_does_the_protocol_have_the_power_it_needs/R45_protocol_freeze/results/r45_frozen_frame.json`,
...
([r82](E03_the_instrument_was_the_object/A09_can_direction_be_read_from_text_alone/R82_scale_use_by_provenance)).** Across **102,147** released ratings on a 21-point
...
2. **The scale carries far less resolution than its 21 points suggest**
   ([r82](E03_the_instrument_was_the_object/A09_can_direction_be_read_from_text_alone/R82_scale_use_by_provenance)). **29.55%** of all weight mass falls on just **5 or
   10**, and **16.96%** on \|w\| = 10 alone.
   ⚠ **And the magnitudes' predictive contribution is already measured — I nearly rebuilt it.**
   [r32](E02_the_plural_public_dissolved/A04_structured_plurality_or_reliability/R32_channel_decomposition) has the cell: moving from **sign** to **signed magnitude**
...
([r83](E03_the_instrument_was_the_object/A09_can_direction_be_read_from_text_alone/R83_low_magnitude_drop)).** A neutral option only helps if absorbing those ratings does
...
**Power, computed from the release rather than deferred to the pilot ([r61](E03_the_instrument_was_the_object/A08_what_floor_each_counterfactual_requires/R61_s_pre_power)).**
The POST arm's sign marginal is **measured**: **77.01%** positive across 102,147 released ratings
(the count is [r82](E03_the_instrument_was_the_object/A09_can_direction_be_read_from_text_alone/R82_scale_use_by_provenance)'s; r61 stores the marginal, not the total),
...
assumed ([r73](E03_the_instrument_was_the_object/A09_can_direction_be_read_from_text_alone/R73_direction_from_text_alone)).** A worry the design does not address on its own:
...
**Registered directional prediction, from [r74](E03_the_instrument_was_the_object/A09_can_direction_be_read_from_text_alone/R74_specificity_vs_exposure).** Text-only
...
- **The only cross-form evidence available** stays [r46](E03_the_instrument_was_the_object/A07_can_a_local_judge_be_an_instrument/R46_spread_replication) — 83.6%
...
separately, and here is the prediction that says why.** [r75](E03_the_instrument_was_the_object/A09_can_direction_be_read_from_text_alone/R75_menu_read_direction) joined
...
[r76](E03_the_instrument_was_the_object/A09_can_direction_be_read_from_text_alone/R76_absence_cannot_overlap) tested the mechanical rival — an absence has no words to
...
the frozen frame against [r38](E02_the_plural_public_dissolved/A06_does_the_protocol_have_the_power_it_needs/R38_human_sampling_power)'s declared `>= median` rule). The
...
([r90](E03_the_instrument_was_the_object/A10_what_the_resampling_unit_is/R90_resampling_unit), [r93](E03_the_instrument_was_the_object/A10_what_the_resampling_unit_is/R93_clustering_unit_transfers), 2026-07-29). r90
...
**⚠ These three figures have NO persisted artifact** (entry 208, 2026-07-29). `0.707` appears exactly once in this repository — in the table below. No round computed them and no results file stores them, so "measured directly" is a claim about work that was not kept. An independent recomputation now has an artifact — [r100](E03_the_instrument_was_the_object/A11_how_wide_every_interval_really_is/R100_rater_reliability): split-half over raters, 200 draws, Spearman–Brown, 937 prompts with ≥8 rating raters, split-half r = **+0.6667** over 187,400 equal-size half-pairs. It gives **0.607 / 0.673 / 0.755** against the stated **0.644 / 0.707 / 0.783** — same ordering, same shape, systematically **0.03–0.04 lower**. That reads as a methodological difference (agreement statistic, prompt filter, or tie handling), **not** a fabrication, and it cannot be adjudicated because the original code does not exist. **⚠ The gap is now attributed** ([r101](E03_the_instrument_was_the_object/A11_how_wide_every_interval_really_is/R101_reliability_offset)): varying r100's three stated choices one at a time, **scoring tied response pairs as 0.5 rather than as agreement** gives **0.7040 at k=8 — within 0.003 of the stated 0.707**, while the tie-drop and both qualifying-set variants stay 0.013–0.035 away. The mechanism is coherent: counting ties as agreement compresses the score's variance and so *lowers* the split-half correlation (0.6667 → 0.6978). **So the stated table is consistent with a half-credit tie rule**, which is a defensible convention and not an error — it is simply not the one r100 used. This is *consistent with*, never an identification: several methods can share a value and the original is gone. **Read the table as an estimate whose method is now narrowed to one choice**; the choice of 8 raters/prompt is unaffected, since both figures sit in the same range.
...
([r64](E03_the_instrument_was_the_object/A08_what_floor_each_counterfactual_requires/R64_satisfaction_substudy_power)).** The earlier one-arm version could not have
...
- **Constructibility screen, before anything else, with its rate as a headline.** Each criterion is
  ...
  [r65](E03_the_instrument_was_the_object/A08_what_floor_each_counterfactual_requires/R65_edit_symmetry_floor) measures a **floor of 18.62%**: that share of core criteria
...
2. **No optional stopping.** n fixed from a pilot, before the main collection.
   **⚠ And δ = 0.01 must be sized, not merely declared** ([r91](E03_the_instrument_was_the_object/A10_what_the_resampling_unit_is/R91_precision_budget),
   ...
   **verified, not assumed**, by [r89](E03_the_instrument_was_the_object/A10_what_the_resampling_unit_is/R89_floor_draw_at_panel_size) across three panel
   ...
   **0.0493** — and this document's power section, written independently and before r91 existed,
   ...
   - **Part of the cost is compute, not annotators.** The donor draw is **20%** of the attribution
     ...
     ([r88](E03_the_instrument_was_the_object/A10_what_the_resampling_unit_is/R88_donor_draw_variance)). Averaging **m = 10** draws cuts the requirement
     ...
     **⚠ Those two figures are the RELEASE's. Restated at the frozen frame**
     ([r94](E03_the_instrument_was_the_object/A10_what_the_resampling_unit_is/R94_donor_averaging_at_frame_size), 2026-07-29): the requirement **transfers**, and
...
7. **δ = 0.01 is a stipulation, now ANCHORED to a decision this project can point at.** It is
   ...
   **⚠ And it fixes something else, which follows from H_fresh's own margin rather than from δ**
   ([r94](E03_the_instrument_was_the_object/A10_what_the_resampling_unit_is/R94_donor_averaging_at_frame_size), 2026-07-29). H_fresh resolves **0.0446** at the
