...
def main() -> None:
    ...
    ap.add_argument("--source", type=Path,
                    default=_ROOT / "E01_the_rubric_was_the_object/A03_is_the_attribution_real_and_against_what_floor/R10_attribution_robustness/results/a10_attribution.json")
    ap.add_argument("--out", type=Path, default=_RES / "r19_floor_choice.json")
    a = ap.parse_args()

    if not a.source.exists():
        raise SystemExit(f"missing {a.source} -- run E01_the_rubric_was_the_object/A03_is_the_attribution_real_and_against_what_floor/R10_attribution_robustness/run.py first")
