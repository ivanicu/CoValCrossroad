"""R280 — the whole evidence table at EVERY annotator, with each cell carrying its own MDE.

WHY. R279 found the release ships a median of 16 annotators per prompt (15,593 annotations) and
...
could be -- its POINT ESTIMATES are drawn from a 3-of-16 subsample, and R278's resolvability verdicts
...
TWO CORRECTIONS IN ONE ROUND, both mechanical, neither optional:
  ...
  (2) give each cell ITS OWN MDE from R279's decomposition, since sigma_b and sigma_w differ by
...
ESTIMAND        (a) A2 per arm, averaged over EVERY annotator of every prompt;
                ...
                (d) which of R278's verdicts change.
IDENTIFICATION  exact for (a) and (b). (c) is identified per R279 and its estimator carries that
...
# R278's 3-draw effects, for the side-by-side. Read from that round's artifact, not retyped.
PRIOR = json.loads((ROOT / "E05_the_space_of_compilers/A16_what_the_definition_costs"
                    / "R278_is_the_boundary_resolvable/results/boundary.json").read_text())
