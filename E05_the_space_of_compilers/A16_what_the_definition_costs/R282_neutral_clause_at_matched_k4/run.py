"""R282 — the neutral clause at EXACTLY matched size, for every selection rule the release allows.

WHY THIS RUNS WHILE R281's JOB IS STILL ON THE GPU. R281 needs a generic arm at k=15 to test `full`.
...
THE QUESTION THIS SEPARATES FROM R281's. R281 asks *does criterion COUNT explain the price*. This
...
ESTIMAND        for each k=4 arm, the neutral gap = A2(arm) - A2(generic), all annotators, with a
                ...
                cell's own MDE (R279's decomposition, per cell).
