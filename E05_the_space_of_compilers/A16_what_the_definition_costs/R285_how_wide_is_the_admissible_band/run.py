...
bottom (A2 = 0.5 by construction of a sign comparison) and the HUMAN CEILING at the top. R280
...
NOISE FLOOR     the per-prompt MDE distribution from R280's 45 cells, carried in and cited, not
                re-derived.
...
R280 = json.loads((ROOT / "E05_the_space_of_compilers/A16_what_the_definition_costs"
                   / "R280_the_table_at_every_annotator/results/all_annotators.json").read_text())
...
def main():
    ...
    mdes = sorted(c["mde"] for c in R280["cells"].values())
    ...
    print(f"    R280 per-cell MDE: median {mde_med:.4f}, range {mdes[0]:.4f}–{mdes[-1]:.4f}")
