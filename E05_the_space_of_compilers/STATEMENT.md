...
| **2** | ③ is **three clauses, not one**: ③-rank (extension **5**†), ③-any (**0**†), ③-judge (**0**†) *(R529, R534)*. **† THREE AXES MEASURED, ONE UNTESTED** *(R580)*: **`A2` target** — at `A1` the extension is **∅** *(R558)* · **`POOL[0:4]` baseline** — the extension moves **4 → 8** across its reference class *(R527)* · **population** — varied across 6 artifacts. ⚠ **`regime` (k) has NO measurement against this number**, and is listed as untested rather than as a condition, because asserting conditioning nobody measured is the flattering error. | the same 41 arms; ③-any and ③-judge coincide *here* only because no ②-passer reads satisfaction |
