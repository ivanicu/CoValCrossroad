"""R305 — what design would resolve the boundary, and is there one?

WHY. R304 left the definition's admit/exclude edge unresolved: `full - topwvar_k4` = +0.0089 against
...
WORLDS          W-NOISE       sigma^2_w dominates -> the boundary is resolvable with annotators
                              already in the release, and R304's "unresolved" is a fact about MY
...
KILL            pre-registered: if MDE(inf) > 0.0089 the boundary is declared STRUCTURALLY
                ...
                If MDE(inf) <= 0.0089 then R304's verdict is a defect OF MY DESIGN and I say so in
...
def main():
    ...
    # (R303, R304) draws ONE annotator and scores BOTH arms against it. Predicted 0.1622, observed
    ...
    # ⚠ THE PRE-REGISTERED REFERENCE (0.0089) IS R304's THREE-DRAW ESTIMATE, and this round
    ...
    print(f"  PRE-REGISTERED KILL: MDE(k→∞) = {b['mde_inf']:.4f} > boundary {BOUNDARY:.4f} "
          f"(R304, 3 draws) ?  {structural}   [KNIFE-EDGE]")
    ...
    if structural:
        ...
        print("  -> W-NOISE. R304's `unresolved` is a defect OF MY DESIGN, not of the release.")
