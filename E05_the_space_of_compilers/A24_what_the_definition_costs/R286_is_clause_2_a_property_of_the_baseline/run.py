...
*better than the same NUMBER of criteria that never read the conversation.* R307 then found the
...
SCOPE           population 968 CoVal prompts with >=2 annotators · instrument Qwen3.5-2B-Base,
                ...
                published in R306 · regime k=4 exactly, unweighted, ALL annotators.
...
POSITIVE CTRL   the incumbent quadruple {0,1,2,3} must appear in the enumeration at exactly its
                pool-internal value (0.5504, R307). It can fail on any subset-indexing error, which
...
def main():
    ...
    print(f"  POSITIVE CONTROL  the incumbent quadruple {{0,1,2,3}} is subset #{inc}, "
          f"A2 = {per_sub[inc]:.4f}  (R307 pool-internal 0.5504)  {'PASS' if pos_ok else 'FAIL'}")
    if not pos_ok:
