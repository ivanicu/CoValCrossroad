...
⚠ AND THE SIZE-MATCHED BLIND REFERENCE IS NOW FREE AT ANY k. R307's 16-criterion pool was judged
...
def main():
    tg, _ = load_targets()
    arms = sorted(p.stem[4:] for p in RES.glob("" + _poshelper.S + "*.npz")
                  if not p.stem.startswith("sat08") and p.stem != "" + _poshelper.S + "genericpool16")
    POOL = load_sat(RES / "" + _poshelper.S + "genericpool16.npz")
    S, K = {}, {}
    for a in arms:
        try: S[a] = load_sat(RES / _poshelper.S + f"{a}.npz")
