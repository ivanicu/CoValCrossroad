"""R307 — clause 2 is the poison comparison. What does the definition admit under the neutral one?
...
SCOPE           population 968 CoVal prompts with >=2 annotators · instrument Qwen3.5-2B-Base
                ...
                annotators (R306), unweighted sums, cluster bootstrap over prompts.
...
KILL            pre-registered, two branches:
                ...
                (2) if the neutral curve's k=4 -> k=16 rise exceeds 0.0121 (the per-cell MDE of
                    `generic - full` from R306), the -0.0426 price is declared CONFOUNDED BY SIZE
...
KS = list(range(1, 17))
NSUB = 20
ARMS = {"coval_core": 4, "topw_k4": 4, "gen": 4, "full": 15}     # arm -> median k (R307 preamble)
...
def main():
    ...
    R260N = json.loads((ROOT / "E05_the_space_of_compilers/A23_is_the_admissibility_gate_the_right_gate"
    ...
    print(f"      per prompt ({abs(mean_shift):.4f} at the mean), which is "
          f"{'BELOW' if abs(mean_shift) < 0.0134 else 'ABOVE'} R306's median MDE of 0.0134.")
