"""R304 — is the definition's own ADMIT/EXCLUDE boundary above this design's resolution?

WHY. R303 measured the MDE of the design behind every number in FORMULATION.md: [0.0100, 0.0200]
...
ESTIMAND        (a) every pairwise paired A2 difference among the 10 arms of the evidence table,
                ...
                (b) each cell's |effect| against R303's MDE bracket, giving a three-valued
...
NOISE FLOOR     R303's MDE bracket, measured, carried in rather than re-derived.
...
NBOOT = 2000
Q = 0.05
MDE_LO, MDE_HI = 0.0100, 0.0200                              # R303, carried in
...
def main():
    ...
    print(f"  {len(pids)} prompts · {len(ARMS)} arms · {len(ARMS)*(len(ARMS)-1)//2} pairs · "
          f"MDE carried from R303 [{MDE_LO:.4f}, {MDE_HI:.4f}]\n")
