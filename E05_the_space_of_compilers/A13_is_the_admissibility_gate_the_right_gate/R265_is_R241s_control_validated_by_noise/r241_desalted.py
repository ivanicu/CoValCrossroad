...
import os as _os
DRAWS = int(_os.environ.get("R265_DRAWS", "20"))          # ⚠ parameterised by R265; R241 hard-coded 20
NPERM = 2000
...
def main() -> int:
    ...
    for (pid, arm, which) in list(store):
        ...
        for dd in range(DRAWS):
            r2 = np.random.default_rng((__import__("zlib").crc32(("%s|%s|%d|%s" % (pid, arm, dd, _os.environ.get("R265_SEED","0"))).encode())) % (2 ** 32))
